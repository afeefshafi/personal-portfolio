...
/**
 * Splash screen shows only on first load OR when hard-refresh.
 * Persisted in sessionStorage so page navigation won't show it again.
 */
export default function SplashGate({ children }) {
  ...
  useEffect(() => {
    ...
    setShow(true)
    const t1 = setTimeout(() => setShow(false), 1250)
    const t2 = setTimeout(() => {
